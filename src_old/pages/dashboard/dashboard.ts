...
import { DashboardFilterPage} from './filter/filter';
...
@ViewChild('view') view: TextInput;
  ...
  addFilter(){
    // if (this.serviceForm.value.state=='QUOTATION'){
      let profileModal = this.modal.create(DashboardFilterPage, {fields: this.fields, field_value: this.field_value});
  ...
  openFilter(item){
    ...
    let profileModal = this.modal.create(DashboardFilterPage, fie);
    profileModal.onDidDismiss(data => {
      if (data) {
