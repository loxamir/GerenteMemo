...
// numeral.locale('es');
/**
 * Generated class for the ImporterPage page.
 *
...
export class ImporterPage {
  ...
  private readCsvData() {
    let file = "";
    if (this.navParams.data.docType == 'sale'){
      file = 'assets/ventas.csv';
    } else if (this.navParams.data.docType == 'contact'){
      file = 'assets/contacts.csv';
    } else if (this.navParams.data.docType == 'product'){
      file = 'assets/products.csv';
    } else if (this.navParams.data.docType == 'cash-move'){
      file = 'assets/contas.csv';
    } else if (this.navParams.data.docType == 'sale-line'){
  ...
  validate(){
    ...
    this.csvData.forEach((doc, lines)=>{
      ...
      if (this.navParams.data.docType == 'product'){
      ...
      } else if (this.navParams.data.docType == 'sale'){
    ...
    if (this.navParams.data.docType == 'product'){
      ...
      Promise.all(promise_ids).then(categories=>{
        ...
        this.formatProducts(this.csvData).then((csv: any[])=>{
          ...
          Promise.all(promise2_ids).then(products=>{
            ...
            const alert = this.alertCtrl.create({
              ...
              subTitle: 'Tus Productos han sido importados con exito!',
              buttons: ['OK, Gracias']
            });
            alert.present();
          })
      ...
      this.formatContacts(this.csvData).then((csv: any[])=>{
        ...
        Promise.all(promise2_ids).then(contacts=>{
          this.pouchdbService.getDoc('config.profile').then((config: any)=>{
          ...
          const alert = this.alertCtrl.create({
            title: 'Importación Exitosa!',
            subTitle: 'Tus Contatos han sido importados con exito!',
            buttons: ['OK, Gracias']
          });
          alert.present();
        })
      ...
      this.formatCashMoves(this.csvData).then((csv: any[])=>{
        ...
        Promise.all(promise2_ids).then(contacts=>{
          ...
          const alert = this.alertCtrl.create({
            title: 'Importación Exitosa!',
            subTitle: 'Tus Cuentas han sido importados con exito!',
            buttons: ['OK, Gracias']
          });
          alert.present();
        })
      ...
      this.formatSales(this.csvData).then((csv: any[])=>{
        ...
        Promise.all(promise2_ids).then(sales=>{
          this.pouchdbService.getDoc('config.profile').then((config: any)=>{
            // console.log("config", bigger_code, config.product_sequence);
          ...
          const alert = this.alertCtrl.create({
            title: 'Importación Exitosa!',
            subTitle: 'Tus Ventas han sido importadas con exito!',
            buttons: ['OK, Gracias']
          });
          alert.present();
          console.log("count", count);
      ...
      this.formatSaleLines(this.csvData).then((csv: any[])=>{
        ...
        Promise.all(promise2_ids).then(data=>{
          ...
          const alert = this.alertCtrl.create({
            title: 'Importación Exitosa!',
            subTitle: 'Tus Lineas de Ventas han sido importadas con exito!',
            buttons: ['OK, Gracias']
          });
          alert.present();
          // console.log("count", count);
