...
import { NavController, App, NavParams, LoadingController, AlertController, Events, ToastController, ModalController, TextInput , PopoverController} from '@ionic/angular';
...
import { ServiceInputPage } from './input/input';
import { ServiceWorkPage } from './work/work';
import { ServiceTravelPage } from './travel/travel';
...
@ViewChild('clrequest') clientRequest: TextInput;
...
//   }
  ...
  ionViewWillLoad() {
    ...
    this.serviceForm = this.formBuilder.group({
      ...
      production: new FormControl(this.navParams.data.production||false),
  ...
  ionViewDidLoad() {
    ...
    this.configService.getConfig().then((data) => {
      ...
      if (this._id){
        this.serviceService.getService(this._id).then((data) => {
  ...
  buttonSave() {
    if (this._id){
      this.serviceService.updateService(this.serviceForm.value);
      this.events.publish('open-service', this.serviceForm.value);
      this.serviceForm.markAsPristine();
    } else {
      this.serviceService.createService(this.serviceForm.value).then(doc => {
  ...
  addWork(){
    // if (this.serviceForm.value.state=='QUOTATION'){
      let profileModal = this.modal.create(ServiceWorkPage, {});
  ...
  editWork(item){
    // if (this.serviceForm.value.state=='QUOTATION'){
      let profileModal = this.modal.create(ServiceWorkPage, item);
      profileModal.onDidDismiss(data => {
        if (data) {
  ...
  editWorkPrice(item){
    if (this.serviceForm.value.state!='CONFIRMED'){
      let prompt = this.alertCtrl.create({
        title: 'Precio del servicio por hora',
  ...
  addTravel(){
    // if (this.serviceForm.value.state=='QUOTATION'){
      let profileModal = this.modal.create(ServiceTravelPage, {});
      profileModal.onDidDismiss(data => {
        if (data) {
  ...
  editTravelPrice(item){
    if (this.serviceForm.value.state!='CONFIRMED'){
      let prompt = this.alertCtrl.create({
        title: 'Precio del Viatico por km',
  ...
  editTravel(item){
    // if (this.serviceForm.value.state=='QUOTATION'){
      let profileModal = this.modal.create(ServiceTravelPage, item);
      profileModal.onDidDismiss(data => {
        if (data) {
          Object.keys(data).forEach(key => {
            item[key] = data[key];
  ...
  // editInput(item){
  //   // if (this.serviceForm.value.state=='QUOTATION'){
  //     let profileModal = this.modal.create(ServiceInputPage, item);
  //     profileModal.onDidDismiss(data => {
  ...
  serviceConfirm(){
    ...
    let prompt = this.alertCtrl.create({
      title: 'Estas seguro que deseas confirmar el servicio?',
  ...
  serviceCancel(){
    let prompt = this.alertCtrl.create({
      title: 'Estas seguro que deseas Cancelar el Servicio?',
  ...
  removeQuotes(){
    this.serviceForm.value.planned.forEach(planned => {
      ...
      this.serviceService.deleteService(planned);
    });
    this.serviceForm.patchValue({
      'planned': [],
    });
  ...
  removeStockMoves(){
    ...
    "stock-move", "origin_id", this.serviceForm.value._id).then((docs) => {
      docs.forEach(doc=>{
        this.serviceService.deleteService(doc);
      })
    });
  }
  ...
  selectEquipment() {
    ...
    if (this.serviceForm.value.state!='PAID'){
      return new Promise(resolve => {
        let profileModal = this.modal.create(ServiceEquipmentPage, this.serviceForm.value.equipment);
        profileModal.onDidDismiss(data => {
          //console.log(data);
          if (data) {
            this.serviceForm.patchValue({
