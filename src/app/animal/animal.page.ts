...
export class AnimalPage implements OnInit {
  ...
  goBack() {
    this.navCtrl.navigateBack(['/agro-tabs/animal-list']);
  }
    ...
    reader.onload = (event: Event) => {
      ...
      preview.onload = async function() {
        ...
        let work: any = await self.pouchdbService.createDoc({
          ...
          'animal_id': self.animalForm.value._id,
          'animal_name': self.animalForm.value.name,
          'activity_name': "Foto",
          'activity_id': "activity.picture",
          'note': self.animalForm.value.note,
          '_attachments': attachment,
        })
        self.animalForm.value.note = "";
      }
    };
    ...
    ApiAIPromises.requestText({
      ...
      contexts: [{
        name: "Animal", parameters: {
          "animal_name": this.animalForm.value.name,
          "animal_id": this.animalForm.value._id
        }
      }]
    })
      .then((result) => {
  ...
  async ngOnInit() {
    ...
    if (this._id) {
      this.animalService.getAnimal(this._id).then(async (data) => {
        this.doInfinite(false);
        data.note = '';
        this.animalForm.patchValue(data);
        this.loading.dismiss();
        let rain = await this.animalService.getAnimalRain(this._id);
        if (rain) {
          this.animalForm.value.lastRainDate = rain['date'];
          this.animalForm.value.lastRain = rain['quantity'];
          var date1 = new Date(this.animalForm.value.lastRainDate);
          var date2 = new Date();
  ...
  doInfinite(infiniteScroll) {
    setTimeout(() => {
      this.animalService.getWorksPage(this._id, this.skip).then((works: any[]) => {
        works.forEach(wor => {
          this.animalForm.value.moves.push(wor);
        })
        console.log("this.animalForm.value.moves", this.animalForm.value.moves);
        this.skip += 15;
        if (infiniteScroll) {
  ...
  buttonSave() {
    if (this._id) {
      this.animalService.updateAnimal(this.animalForm.value, this.avatar);
      this.events.publish('open-animal', this.animalForm.value);
      this.showForm = false;
    } else {
      this.animalService.createAnimal(this.animalForm.value, this.avatar).then(doc => {
        this.animalForm.patchValue({
          _id: doc['id'],
        });
        this._id = doc['id'];
  ...
  async addActivity(activity_id) {
    let componentProps = {
      "animal": this.animalForm.value,
      "select": true,
    }
    if (activity_id) {
      componentProps['activity'] = await this.pouchdbService.getDoc(activity_id);
      componentProps['note'] = this.animalForm.value.note;
    }
    let profileModal = await this.modalCtrl.create({
      component: WorkPage,
  ...
  async createPayment() {
    let componentProps = {
      ...
      "select": true,
      "signal": "-",
      "contact_id": this.animalForm.value._id,
    }
    let profileModal = await this.modalCtrl.create({
  ...
  sendButton() {
    ...
    this.pouchdbService.createDoc({
      ...
      'animal_id': this.animalForm.value._id,
      'animal_name': this.animalForm.value.name,
      // 'crop_id': this.animalForm.value.crop._id,
      // 'crop_name': this.animalForm.value.crop.name,
      'activity_name': "Anotación",
      'activity_id': "activity.anotation",
      'note': this.animalForm.value.note,
    })
    this.animalForm.value.note = '';
  }
  ...
  deleteWork(work) {
    // let index = this.animalForm.value.moves.indexOf(work);
