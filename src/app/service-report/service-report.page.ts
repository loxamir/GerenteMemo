...
export class ServiceReportPage implements OnInit {
    ...
    return new Promise(resolve => {
      if (this.reportServiceForm.value.groupBy == 'service') {
        ...
        this.pouchdbService.getView(
          ...
          let promise_ids = [];
          let result = {};
          if (this.reportServiceForm.value.groupBy == 'category') {
            items = [];
            let getList = [];
            services.forEach(serviceLine => {
            ...
            items = [];
            services.forEach(serviceLine => {
              if (result.hasOwnProperty(serviceLine.key[1])) {
                // console.log("items[result[serviceLine.key[1]]]", items[result[serviceLine.key[1]]]);
                items[result[serviceLine.key[1]]] = {
                  'name': items[result[serviceLine.key[1]]].name,
                  'quantity': items[result[serviceLine.key[1]]].quantity + parseFloat(serviceLine.key[4]),
                  'margin': items[result[serviceLine.key[1]]].margin + parseFloat(serviceLine.key[3]),
                  'total': items[result[serviceLine.key[1]]].total + parseFloat(serviceLine.key[4])*serviceLine.key[5],
                };
              } else {
                items.push({
                  'name': serviceLine.key[1],
                  'quantity': parseFloat(serviceLine.key[4]),
                  'margin': parseFloat(serviceLine.key[3]),
                  'total': parseFloat(serviceLine.key[4])*serviceLine.key[5],
                });
                result[serviceLine.key[1]] = items.length-1;
              }
            });
  ...
  orderByName() {
    this.reportServiceForm.patchValue({
      'orderBy': "name",
    })
    this.goNextStep();
  }
  ...
  goNextStep() {
    this.getData().then(data => {
      ...
      new Promise((resolve, reject) => {
        self.reportServiceForm.patchValue({
          "items": data,
        });
        resolve(true);
      }).then(() => {
  ...
  recomputeValues() {
    ...
    this.reportServiceForm.value.items.forEach((item) => {
      total += parseFloat(item.total);
  ...
  drawNewBar() {
    let self = this;
    var dataset = this.reportServiceForm.value.items;
    var width = 320;
    var height = 200;
    ...
    var path = svg.selectAll(".bar")
      .data(this.reportServiceForm.value.items)
      .enter().append("rect")
      .attr("class", "bar")
