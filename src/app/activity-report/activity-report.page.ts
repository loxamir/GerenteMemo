...
export class ActivityReportPage implements OnInit {
  ...
  recomputeValues() {
    ...
    let planned = 0;
    let planned_yield = 0;
    this.items = this.reportActivityForm.value.items;
    ...
    this.drawPie();
    if (this.reportActivityForm.value.groupBy == 'area'){
      this.drawNewBar2();
  ...
  drawNewBar2() {
    ...
    this.y = d3Scale.scaleLinear().rangeRound([height, -80]);
    this.x.domain(dataset.map((d: any) => this.translate.instant(d.name)));
    this.y.domain([0, d3Array.max(dataset, (d: any) => d.total/d.quantity)]);
    ...
    var path = svg.selectAll(".bar")
      ...
      .attr("class", "bar")
      .attr("x", (d: any) => this.x(this.translate.instant(d.name)))
      .attr("y", (d: any) => this.y(d.total/d.quantity))
      .attr("width", this.x.bandwidth())
      .style("fill", (d: any) => color(d['name']))
      .attr("height", (d:any) => height - this.y(d.total/d.quantity))
      .each(function(d:any) { self._current - d; });

    path.on('mouseover', (d:any) =>{  // when mouse enters div
      d3.sum(dataset.map(function(d:any) { // calculate the total number of tickets in the dataset
        return (d.enabled) ? d.total/d.quantity : 0; // checking to see if the entry is enabled. if it isn't, we return 0 and cause other percentages to increase
      }));
      tooltip.select('.label').html(this.translate.instant(d.name)); // set current label
      tooltip.select('.count').html('$' + (d.total/d.quantity).toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ".")); // set current count
      tooltip.style('display', 'block'); // set display
    });
  ...
  drawNewBar3() {
    ...
    this.y = d3Scale.scaleLinear().rangeRound([height, -80]);
    this.x.domain(dataset.map((d: any) => this.translate.instant(d.name)));
    this.y.domain([0, d3Array.max(dataset, (d: any) => d.margin/d.area)]);
    ...
    var path = svg.selectAll(".bar")
      ...
      .attr("class", "bar")
      .attr("x", (d: any) => this.x(this.translate.instant(d.name)))
      .attr("y", (d: any) => this.y(d.margin/d.area))
      .attr("width", this.x.bandwidth())
      .style("fill", (d: any) => color(d['name']))
      .attr("height", (d:any) => height - this.y(d.margin/d.area))
      .each(function(d:any) { self._current - d; });
