...
export class AccountCategoryListPage implements OnInit {
  ...
  doInfinite(infiniteScroll) {
    setTimeout(() => {
      this.getAccountCategorysPage(this.searchTerm, this.page).then((accountCategorys: any[]) => {
        // this.accountCategorys = accountCategorys
  ...
  doRefresh(refresher) {
    setTimeout(() => {
      this.getAccountCategorysPage(this.searchTerm, 0).then((accountCategorys: any[]) => {
        if (this.filter == 'all'){
          this.accountCategorys = accountCategorys;
        }
  ...
  openAccountCategory(accountCategory) {
    ...
    this.navCtrl.navigateForward(['/account-category', {'_id': accountCategory._id}]);
  }
