...
export class ProductionReportPage implements OnInit {
    ...
    return new Promise(resolve => {
      if (this.reportProductionForm.value.reportType == 'production') {
        this.pouchdbService.getView(
          ...
          let promise_ids = [];
          let result = {};
          if (this.reportProductionForm.value.groupBy == 'category') {
            items = [];
            let getList = [];
  ...
  orderByName() {
    this.reportProductionForm.patchValue({
      'orderBy': "name",
    })
    this.goNextStep();
  }
  ...
  ngOnInit() {
    ...
    if (this._id) {
      this.reportService.getReport(this._id).then((data) => {
        ...
        this.reportProductionForm.patchValue(data);
        //this.loading.dismiss();
      });
    } else {
      //this.loading.dismiss();
  ...
  goNextStep() {
    this.getData().then(data => {
      ...
      new Promise((resolve, reject) => {
        self.reportProductionForm.patchValue({
          "items": data,
        });
        resolve(true);
      }).then(() => {
  ...
  recomputeValues() {
    ...
    this.reportProductionForm.value.items.forEach((item) => {
      total += parseFloat(item.total);
  ...
  drawNewBar() {
    let self = this;
    var dataset = this.reportProductionForm.value.items;
    var width = 320;
    var height = 200;
    ...
    var path = svg.selectAll(".bar")
      .data(this.reportProductionForm.value.items)
      .enter().append("rect")
      .attr("class", "bar")
